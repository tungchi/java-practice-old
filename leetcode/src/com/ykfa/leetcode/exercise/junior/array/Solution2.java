package com.ykfa.leetcode.exercise.junior.array;

/**
 * @author yingkefa
 * @date 2020年03月06日16:49:13
 */
        ...
        for (int i = 0; i < prices.length - 1; i++) {
            if (status == 0) {
                if (prices[i] < prices[i + 1]) {
                    buyPrice = prices[i];
                    status = 1;
                    System.out.println("第" + (i + 1) + "天买入,价格为" + buyPrice);
                }
            } else {
                if (buyPrice < prices[i] && prices[i] > prices[i + 1]) {
                    status = 0;
                    System.out.println("第" + (i + 1) + "天卖出,价格为" + prices[i + 1]);
                }
            }
        }
        return 0;
